...
class FFM(nn.Module):
    def __init__(self, embedding_size, field_size, feature_size, device):
    ...
    def forward(self, features):
        ...
        for i in range(self.field_size):
            for j in range(i + 1, self.field_size):
                ...
                second_order += torch.sum(torch.mul(vifj, vjfi), dim=1, keepdim=True) * feature_values[:, i][:,
                                                                                        np.newaxis] * feature_values[:,
                                                                                                      j][:, np.newaxis]
